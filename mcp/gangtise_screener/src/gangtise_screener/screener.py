...
def _format_universe_summary(res: dict) -> str:
    ...
    csv_sources = uni.get("csv_sources") or []
    parts: List[str] = []
    if csv_sources:
        for s in csv_sources:
            path = s.get("path") or ""
            base = os.path.basename(path) if path else "csv"
            parts.append(f"CSV `{base}`（{s.get('count')}只）")
    # 大量证券代码时不逐条展开；非 id（板块/别名等）仍展示
    show_all = len(details) <= 8 and not csv_sources
    for d in details:
        kind = d.get("kind") or ""
        if csv_sources and kind == "id":
            continue
        if not show_all and kind == "id":
            continue
    ...
    if parts:
        summary = "、".join(parts)
        ids = uni.get("universe") or (res.get("payload") or {}).get("universe") or []
        if csv_sources and ids and f"{len(ids)}只" not in summary:
            summary += f"，合计 {len(ids)} 只"
    else:
        payload = res.get("payload") or {}
        ids = uni.get("universe") or payload.get("universe") or []
        if len(ids) > 8:
            summary = "、".join(f"`{u}`" for u in ids[:3]) + f" … 共 {len(ids)} 只"
        else:
...
def format_resolve_universe_markdown(res: dict) -> str:
    ...
    status = res.get("status") or "?"
    if status == "ok":
        uni = res.get("universe") or []
        lines = ["## 范围解析\n", f"- **状态**：`ok`"]
        for s in res.get("csv_sources") or []:
            lines.append(f"- **CSV**：`{s.get('path')}`（{s.get('count')}只）")
        if len(uni) > 12:
            sample = ", ".join(f"`{u}`" for u in uni[:5])
            lines.append(f"- **universe**：{sample} … 共 {len(uni)} 只")
        else:
            lines.append(f"- **universe**：{', '.join(f'`{u}`' for u in uni)}")
        for d in res.get("details") or []:
            kind = d.get("kind") or ""
            if kind == "id" and res.get("csv_sources") and len(uni) > 8:
                continue
            resolved = d.get("resolved") or {}
            lines.append(
...
def main():
    ...
    p2.add_argument(
        "-u", "--universe", action="append", default=None,
        help="选股范围（可选，默认全A）：板块名/别名/sectorId/证券代码；"
             "可重复或逗号分隔；亦可传 .csv 路径（读 code/security_code/代码 列）",
    )
