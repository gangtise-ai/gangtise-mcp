...
from gangtise_pdf.tools_registry import INTERNAL_PARAMS, TOOL_HANDLERS
...
def _check_auth_configured() -> Optional[str]:
    ...
    return (
        ...
        f"配置方式：gangtise-pdf configure --access-key <AK> --secret-key <SK>"
...
def _build_epilog(spec_map: Dict[str, ToolSpec]) -> str:
    lines = ["子命令一览（gangtise-pdf <命令> --help 查看参数）:", ""]
    listed: Set[str] = set()
    ...
    if other:
        ...
        lines.append("")
    lines.append("示例:")
    lines.append("  gangtise-pdf configure --access-key <AK> --secret-key <SK>")
    lines.append("  gangtise-pdf pdf_parse --action submit --pdf-path ./sample.pdf")
    lines.append("  gangtise-pdf pdf_parse --action result --task-id <task_id>")
    lines.append("  gangtise-pdf list")
    return "\n".join(lines)


def _cmd_list(spec_map: Dict[str, ToolSpec]) -> int:
    print(_build_epilog(spec_map).replace("子命令一览（gangtise-pdf <命令> --help 查看参数）:", "可用命令:"))
    return 0
...
def _cmd_configure(args: argparse.Namespace) -> int:
    ...
    if args.show:
        if is_auth_configured():
            ...
            print(f"未配置授权。运行 gangtise-pdf configure --access-key <AK> --secret-key <SK>")
        return 0
...
def _build_parser(spec_map: Dict[str, ToolSpec]) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--version",
        action="version",
        version="gangtise-pdf-mcp 0.1.0",
    )
